...
class VendingMachine:
    ...
    def restock(self,adding):
        self.number+=adding
        return f'Current {self.name} stock: {self.number}'
    
    def vend(self):
        if self.fund==0 and self.number==0:
            return 'Nothing left to vend. Please restock.'
        else:
            gap=self.price-self.fund
            if gap>0:
                return f'Please add ${gap} more funds.'
            else:
                self.number-=1
                self.fund=0
                if gap==0:
                    ...
                    return f'Here is your {self.name} and ${-gap} change.'
...
def two_list(vals, counts):
    ...
    vals_rev=vals[::-1]
    counts_rev=counts[::-1]
    part=Link(vals_rev[0])
    counts_rev[0]-=1
    for i in range(len(vals_rev)):
        if counts_rev[i]!=0:
            for j in range(counts_rev[i]):
                part=Link(vals_rev[i],part)
    return part
