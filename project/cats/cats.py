...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def feline_basic(typed, source, changes):
        if changes > limit:
            return limit + 1  
        if len(typed) == 0:
        ...
        if typed[0] == source[0]:
            return feline_basic(typed[1:], source[1:], changes)
        else:
            return feline_basic(typed[1:], source[1:], changes + 1)
    
    return feline_basic(typed, source, 0)
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times=[]
    for item in timestamps_per_player:
        times_player=[]
        for i in range(1,len(item)):
            times_player.append(item[i]-item[i-1])
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        for k in player_indices:
            if times[k][j]<times[min_player_index][j]:
                min_player_index=k
        answer[min_player_index].append(words[j])
    return answer
    # END PROBLEM 10
